...
FEATURE_NAMES = [ "Blue (B)", "Green (G)", "Red (R)", "NIR", "NDWI", "NDVI", "Slope", "Aspect"]
...
def load_masks_and_compute_stats(territory):
    ...
    for f in tqdm(mask_files, desc=f"Processing {territory}"):
        with rasterio.open(f) as src:
            ...
            stats.append({"file": f.name, "date": date_str, "obj_pixels": obj_pixels, "obj_area_ha": obj_area_ha, "total_pixels": mask.size, "fraction": obj_pixels / mask.size if mask.size > 0 else 0})
...
def plot_feature_importance(model, territory):
    ...
    # Если признаков больше, чем исходных, то создаём имена для текстур
...
def plot_training_curve(model, oob_scores=None): # not usable, TL;DR
    if hasattr(model, 'oob_score_'):
        ...
        if oob_scores is not None and len(oob_scores) > 0:
            ...
            print("Для построения кривой OOB необходимо передать список oob_scores.")
    else:
        print("Убедитесь, что в конфиге RandomForestClassifier oob_score=True.")
...
def main():
    ...
    for territory in ["Amga", "Yunkor"]:
        try:
            ...
            print(f"{territory} - {STATS_DIR / f'{territory}_stats.csv'}")
